    ...
    public class TerrainUVUtil : ScriptableObject
    {
        ...
        private void GetTerrainInfo(GameObject[] terrainObjects)
        {
            ...
            for (int i = 0; i < terrainObjects.Length; i++)
            {
                ...
                //Set height to highest terrain
                if (terrain.terrainData.bounds.size.y > height)
                {
                    height = terrain.terrainData.bounds.size.y;
                }
            ...
            centerPostion = originPosition + (size / 2f);

            pigmentMapSize = terrains[0].terrainData.alphamapWidth;

            terrainScaleOffset = new Vector4(size.x, size.z, Mathf.Abs(originPosition.x), Mathf.Abs(originPosition.z));
